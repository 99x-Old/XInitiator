...
import { saveNewInitiative } from '../../../services/InitiativeService'
...
export default function AddInitiative() {
    ...
    useEffect(() => {
        if (initiativeName !== "" && initiativeDescription !== "") {
            setFilled(true);
        } else {
            setFilled(false);
        }
    ...
    const saveInitiative = () => {
        ...
        saveNewInitiative(JSON.stringify(body)).then(response => {
        ...
        <Grid container style={{ marginTop: '5%', width: '100%' }}>
            ...
            <Grid item xs={6}>
                <Card style={{ width: '100%' }}>
                    ...
                    <CardActions>
                        <FormControl fullWidth style={{ marginTop: 20 }}>
                            <Button size="large" disabled={!filled} variant="contained" onClick={saveInitiative} color="primary">Save</Button>
                            <br />
